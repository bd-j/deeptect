...
if __name__ == "__main__":
    ...
    n_pix_per_side_withbound = int(n_pix_per_side + 2*n_pix_per_gal)

    # get coordinates
    cat["x"] = np.floor(np.random.uniform(low=1.0 + n_pix_per_gal, high=n_pix_per_side_withbound - n_pix_per_gal - 1.0, size=n_gal))
    cat["y"] = np.floor(np.random.uniform(low=1.0 + n_pix_per_gal, high=n_pix_per_side_withbound - n_pix_per_gal - 1.0, size=n_gal))

    # make image
    im = make_image(cat, n_pix_per_side_withbound, n_pix_per_gal,
                    sigma_psf=args.PSF_SIGMA, pixel_scale=args.PIXEL_SCALE)
    truth = im.copy().array[int(n_pix_per_gal):-int(n_pix_per_gal), int(n_pix_per_gal):-int(n_pix_per_gal)]
    ...
    write_im("../data/{}_truth.fits".format(args.name), truth, header, NOISE=0)
    write_im("../data/{}_noisy.fits".format(args.name), noisy, header)
    write_im("../data/{}_unc.fits".format(args.name), args.NOISE*np.ones_like(truth), header)
